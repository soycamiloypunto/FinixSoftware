$tsPath = "d:\CCTV\Projects\FinixSoftware\finix_frontend\src\app\features\egreso\egreso\egreso.ts"
...
$ts = $ts -replace "egresos = signal<EgresoModel\[\]>\(\[\]\);", "dataSource = new MatTableDataSource<EgresoModel>([]);`n    @ViewChild(MatPaginator) paginator!: MatPaginator;`n    fechaFiltro: string = new Date().toISOString().substring(0,10);"
$ts = $ts -replace "implements OnInit", "implements OnInit, AfterViewInit"
$ts = $ts -replace "cargarEgresos\(\) \{[\s\S]*?\}", "ngAfterViewInit() { this.dataSource.paginator = this.paginator; }`n`n    applyFilter(event: Event) {`n      const filterValue = (event.target as HTMLInputElement).value;`n      this.dataSource.filter = filterValue.trim().toLowerCase();`n    }`n`n    cargarEgresos() {`n      const start = new Date(this.fechaFiltro + 'T00:00:00-05:00').toISOString();`n      const end = new Date(this.fechaFiltro + 'T23:59:59-05:00').toISOString();`n      this.egresoService.getEgresosByDateRange(start, end).subscribe(data => this.dataSource.data = data);`n    }"
$ts = $ts -replace "egresos\(\)", "dataSource.data"
