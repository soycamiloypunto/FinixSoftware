...
ts = ts.replace("const sesionesFinalizadasUI = this.mapToSesionUI(sesionesFinalizadas);\r\n        this.sesionesFinalizadas.set(sesionesFinalizadasUI);", "const sesionesFinalizadasUI = this.mapToSesionUI(sesionesFinalizadas);\n        this.sesionesFinalizadas.set(sesionesFinalizadasUI);\n        this.dataSourceFinalizadas.data = sesionesFinalizadasUI;");
ts = ts.replace("const sesionesFinalizadasUI = this.mapToSesionUI(sesionesFinalizadas);\n        this.sesionesFinalizadas.set(sesionesFinalizadasUI);", "const sesionesFinalizadasUI = this.mapToSesionUI(sesionesFinalizadas);\n        this.sesionesFinalizadas.set(sesionesFinalizadasUI);\n        this.dataSourceFinalizadas.data = sesionesFinalizadasUI;");
...
html = html.replace('Historial de Tiempos Cobrados\r\n            </h2>', `Historial de Tiempos Cobrados\r\n            </h2>\n      <div class="flex items-center space-x-4 mb-4">\n          <input type="date" [(ngModel)]="fechaFiltro" (change)="cargarHistorial()" class="p-2 rounded border bg-white text-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-500">\n          <input type="text" (keyup)="applyFilter($event)" placeholder="Buscar..." class="p-2 rounded border bg-white text-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 w-full sm:w-1/2">\n      </div>`);
html = html.replace('Historial de Tiempos Cobrados\n            </h2>', `Historial de Tiempos Cobrados\n            </h2>\n      <div class="flex items-center space-x-4 mb-4">\n          <input type="date" [(ngModel)]="fechaFiltro" (change)="cargarHistorial()" class="p-2 rounded border bg-white text-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-500">\n          <input type="text" (keyup)="applyFilter($event)" placeholder="Buscar..." class="p-2 rounded border bg-white text-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 w-full sm:w-1/2">\n      </div>`);
...
html = html.replace('<ng-container matColumnDef="total">', `<ng-container matColumnDef="productosAdicionales">\n              <th mat-header-cell *matHeaderCellDef class="font-semibold text-gray-600 dark:text-gray-300"> Subproductos (Pedidos) </th>\n              <td mat-cell *matCellDef="let element" class="text-gray-800 dark:text-gray-200 py-3">\n                 <ul class="text-xs m-0 pl-4 list-disc" *ngIf="element.productosAdicionales && element.productosAdicionales.length > 0">\n                    <li *ngFor="let prod of element.productosAdicionales">\n                       {{prod.cantidad}}x {{prod.nombreProducto || prod.producto?.nombre}} (\${{prod.total | number:'1.0-0'}})\n                    </li>\n                 </ul>\n                 <span *ngIf="!element.productosAdicionales || element.productosAdicionales.length === 0" class="text-gray-400 italic">Ninguno</span>\n              </td>\n            </ng-container>\n\n            <ng-container matColumnDef="total">`);
